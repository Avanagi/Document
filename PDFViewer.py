...
class PDFViewer:
    def __init__(self, master):
        ...
        self.numPages = None
        self.master = master
        self.text = ""  # Штука для анализа текста, возможно
        ...
        self.highlight_canvas = None  # Инициализация
    ...
    def accept_text(self):
        ...
        if input_text.lower() == "головная":
            ...
            cleaned_line = cleaned_text[1] + ' '.join([part if part.endswith('-') else f' {part}' for part in cleaned_text[2:]])
        ...
        if event_type == EventType.ButtonPress:
            ...
            if self.highlight_canvas:
                ...
                if any([self.x_p is not None, self.y_p is not None, self.x_r is not None, self.y_r is not None]):
                    ...
                    # Проверяем, чтобы хотя бы одна точка находилась в пределах канваса
                    canvas_width = self.output.winfo_width()
                    canvas_height = self.output.winfo_height()

                    if x1 < 0 or y1 < 0 or x2 > canvas_width or y2 > canvas_height:
                        # Один из углов выходит за пределы канваса, корректируем координаты
    ...
    def save_rectangle(self, x1, y1, x2, y2, filename):
        try:
            ...
            if x2 > x1 and y2 > y1:
                # Получаем область экрана с координатами x1, y1, x2, y2
                ...
                self.text = pytesseract.image_to_string(image, lang='rus')  # Сохраняем текст в атрибут
                if self.text:
